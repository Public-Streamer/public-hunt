-- Generate comprehensive test data for Public Streamer MVP - Fixed version
...
SELECT 
  ...
  CASE 
    WHEN au.email LIKE 'advertiser%' THEN 'Professional advertiser specializing in ' || (ARRAY['tech', 'lifestyle', 'gaming', 'fitness', 'travel', 'food', 'fashion', 'music'])[floor(random() * 8 + 1)::int] || ' content.'
    ELSE 'Content creator passionate about ' || (ARRAY['live streaming', 'gaming', 'music', 'art', 'cooking', 'fitness', 'tech reviews', 'travel'])[floor(random() * 8 + 1)::int] || '.'
  END,
  (ARRAY['New York, NY', 'Los Angeles, CA', 'Chicago, IL', 'Houston, TX', 'Phoenix, AZ', 'Philadelphia, PA', 'San Antonio, TX', 'San Diego, CA', 'Dallas, TX', 'Austin, TX'])[floor(random() * 10 + 1)::int],
  CASE 
...
ad_data AS (
  SELECT 
    ...
    au.username,
    i as ad_number,
    (ARRAY['video', 'image', 'carousel'])[floor(random() * 3 + 1)::int] as ad_type,
    (ARRAY['Tech & Gadgets', 'Fashion & Style', 'Health & Fitness', 'Food & Dining', 'Travel & Adventure', 'Gaming', 'Music & Entertainment', 'Home & Garden', 'Sports', 'Education'])[floor(random() * 10 + 1)::int] as category,
    (ARRAY['Brand Awareness', 'Lead Generation', 'Sales', 'Traffic', 'Engagement'])[floor(random() * 5 + 1)::int] as campaign_goal
...
SELECT 
  ...
  (ARRAY['young adults', 'professionals', 'families', 'students', 'seniors'])[floor(random() * 5 + 1)::int] || 
  ...
  CASE 
    ...
    WHEN ad.ad_type = 'carousel' THEN ARRAY['https://picsum.photos/800/600?random=' || floor(random() * 1000)::int, 'https://picsum.photos/800/600?random=' || floor(random() * 1000)::int]
    ELSE ARRAY['https://picsum.photos/800/600?random=' || floor(random() * 1000)::int]
  END,
  NOW() - (random() * interval '6 months'),
  NOW() - (random() * interval '7 days')
FROM ad_data ad;
...
SELECT 
  ...
  floor(random() * 5 + 1)::int, -- 1-5 star rating
...
AND random() < 0.5 -- 50% of ads get feedback
LIMIT 1000;
...
SELECT 
  ...
  cu.user_id,
  cu.display_name || '''s ' || 
  (ARRAY['Gaming', 'Music', 'Tech', 'Cooking', 'Fitness', 'Art', 'Travel', 'Education', 'Comedy', 'Sports'])[floor(random() * 10 + 1)::int] || 
  ' Channel ' || channel_num,
  'Welcome to my channel! I create ' || 
  (ARRAY['amazing live content', 'interactive experiences', 'educational content', 'entertaining shows', 'community events'])[floor(random() * 5 + 1)::int] ||
  ' focused on ' || 
  (ARRAY['gaming', 'technology', 'lifestyle', 'creativity', 'learning', 'entertainment'])[floor(random() * 6 + 1)::int] ||
  '. Join our community and don''t miss any live streams!',
  (ARRAY['Gaming', 'Music', 'Technology', 'Cooking', 'Fitness', 'Art', 'Travel', 'Education', 'Comedy', 'Sports', 'Lifestyle', 'Business'])[floor(random() * 12 + 1)::int],
  split_part(cu.display_name, ' ', 1),
  split_part(cu.display_name, ' ', 2),
  'creator' || split_part(cu.username, 'creator', 2) || '@publicstreamer.com',
  ARRAY['https://picsum.photos/800/600?random=' || floor(random() * 1000)::int],
  NOW() - (random() * interval '1 year'),
  NOW() - (random() * interval '7 days')
...
WHERE au.created_at > NOW() - interval '1 minute'
AND random() < 0.05 -- 5% subscription rate to limit data
LIMIT 2000;
...
SELECT 
  gen_random_uuid(),
  (ARRAY['Live', 'Special', 'Weekly', 'Premium', 'Exclusive'])[floor(random() * 5 + 1)::int] || ' ' ||
  c.category || ' ' ||
  (ARRAY['Show', 'Stream', 'Event', 'Session', 'Experience'])[floor(random() * 5 + 1)::int] || ' ' ||
  event_num,
  'Join us for an incredible ' || c.category || ' experience! ' ||
  (ARRAY[
  ...
  (ARRAY['Online', 'New York', 'Los Angeles', 'Chicago', 'Virtual Studio', 'Home Studio'])[floor(random() * 6 + 1)::int],
  c.category,
  CASE 
  ...
  c.user_id,
  c.id,
  floor(random() * 200 + 50)::int, -- 50-250 max participants
  random() < 0.1, -- 10% currently live for more test data
  floor(random() * 1000)::int, -- 0-999 viewers
  (ARRAY['HD', '4K', 'HD', 'SD'])[floor(random() * 4 + 1)::int],
  NOW() - (random() * interval '6 months'),
  NOW() - (random() * interval '7 days'),
