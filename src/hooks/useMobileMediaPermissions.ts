...
export interface MobileMediaPermissions {
  ...
  requestCameraPermission: (facingMode?: "user" | "environment") => Promise<boolean>;
  requestMicrophonePermission: () => Promise<boolean>;
  requestBothPermissions: (facingMode?: "user" | "environment") => Promise<{ camera: boolean; microphone: boolean }>;
...
export const useMobileMediaPermissions = (): MobileMediaPermissions => {
  ...
  const requestCameraPermission = useCallback(async (facingMode: "user" | "environment" = "user"): Promise<boolean> => {
    try {
      console.log("📱 MOBILE DEBUG - Requesting camera permission with facingMode:", facingMode);
      ...
      if (error instanceof Error) {
        if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
          ...
          try {
            ...
            console.error("📱 MOBILE DEBUG - Fallback camera request failed:", fallbackError);
            toast.error("Camera constraints not supported. Please try a different device.");
          }
        } else {
  ...
  const requestBothPermissions = useCallback(async (facingMode: "user" | "environment" = "user") => {
    try {
      console.log("📱 MOBILE DEBUG - Requesting both camera and microphone permissions");
      ...
      console.log("📱 MOBILE DEBUG - Falling back to individual permission requests");
  ...
  const requestScreenShare = useCallback(async (): Promise<boolean> => {
    if (!checkScreenShareSupport()) {
      console.log("📱 MOBILE DEBUG - Screen share not supported on this device");
