  ...
  const toggleLike = useCallback(async () => {
    ...
    try {
      ...
      if (userLike) {
        ...
        setUserLike(newLike);
        setLikes((prev) =>
          prev.map((like) => (like.id === tempLike.id ? newLike : like))
        );
        setLoadingLikes(false);
      }
    } catch (error) {
              ...
              comment.id === parentCommentId
                ? {
                    ...
                    replies: (comment.replies || []).map((r) =>
                      r.id === tempComment.id ? newComment : r
                    ),
                  }
                : comment
            )
    ...
    async (commentId: string, parentCommentId?: string) => {
      try {
        ...
        if (parentCommentId) {
          ...
          commentToDelete = parentComment?.replies?.find(
            (r) => r.id === commentId
          );
        } else {
                ...
                prev.map((comment) =>
                  comment.id === newComment.parent_comment_id
                    ? {
                        ...comment,
                        replies: (comment.replies || []).map((r) =>
                          r.id === newComment.id ? newComment : r
                        ),
                      }
                    : comment
                )
