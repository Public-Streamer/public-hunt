...
export const useScoreboardTeams = (
  eventId: string,
  scoreboardType?: string
): UseScoreboardTeamsResult => {
  console.log(
    '[useScoreboardTeams] Hook called with eventId:',
    eventId,
    'scoreboardType:',
    scoreboardType
  );
  ...
  useEffect(() => {
    ...
    console.log(
      '[useScoreboardTeams] Setting up subscription for eventId:',
      eventId,
      'scoreboardType:',
      scoreboardType
    );
    ...
    const fetchTeamCount = async () => {
      ...
      try {
        ...
        if (error) {
          console.error(
            '[useScoreboardTeams] Error fetching team count:',
            error
          );
          return;
        }
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log(
            '[useScoreboardTeams] Real-time teams update received:',
            payload
          );
            ...
            if (scoreboardType) {
              if (
                ...
                if (newRecord?.scoreboard_type !== scoreboardType) {
                  console.log(
                    '[useScoreboardTeams] Ignoring event for different scoreboard type:',
                    newRecord?.scoreboard_type,
                    'vs',
                    scoreboardType
                  );
              ...
              if (payload.eventType === 'DELETE') {
                ...
                if (oldRecord?.scoreboard_type !== scoreboardType) {
                  console.log(
                    '[useScoreboardTeams] Ignoring DELETE for different scoreboard type:',
                    oldRecord?.scoreboard_type,
                    'vs',
                    scoreboardType
                  );
                  return;
    ...
    return () => {
      console.log(
        '[useScoreboardTeams] Cleaning up subscription for eventId:',
        eventId,
        'scoreboardType:',
        scoreboardType
      );
