...
const EnhancedQABlog = () => {
  ...
  const checkForAIResponses = async () => {
    ...
    const postsNeedingAI = blogPosts.filter(post => 
      !post.blog_answers?.some(answer => answer.is_ai_generated)
    );

    for (const post of postsNeedingAI.slice(0, 2)) { // Limit to 2 at a time
      try {
                ...
                <div className="space-y-4">
                  {post.blog_answers?.map((answer) => (
                    <div key={answer.id} className={`border-l-2 pl-4 py-2 ${
                      answer.is_ai_generated ? 'border-purple-200 bg-purple-50/50' : 'border-gray-200'
                    }`}>
