  ...
  return (
    <div className="py-8">
      <div className="container mx-auto px-4">
        <h2 className="text-2xl font-bold mb-6">📺 Live Now - Sorted by Popularity</h2>
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="space-y-4">
              {liveEvents.map((event) => (
                <div key={event.id} className="flex gap-4 p-4 bg-background rounded-lg border hover:shadow-md transition-shadow">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-semibold line-clamp-2 pr-2">{event.title}</h3>
                      <Badge variant="outline" className="flex-none">{event.category}</Badge>
                    </div>
                    
                    <p className="text-sm text-muted-foreground mb-2">{event.creator}</p>
