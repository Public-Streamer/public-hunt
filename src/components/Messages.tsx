  ...
  const fetchApprovalRequests = async () => {
    try {
      ...
      const { data: requests, error } = await supabase
        ...
        .eq('status', 'pending')
        .or(
          `channels.user_id.eq.${userData.user.id},channel_id.in.(select channel_id from channel_permissions where user_id = ${userData.user.id} and role in ('channel_master','channel_admin'))`
        );
  ...
  const fetchConversations = async () => {
    try {
      ...
      if (approvalRequests.length > 0) {
        const approvalConversations: Conversation[] = approvalRequests.map(
          (request) => ({
                    ...
                    >
                      <div className="relative">
                        <Avatar className="w-12 h-12">
                          <AvatarImage
                            src={conv.user_profile.profile_picture_url}
                          />
                          <AvatarFallback>
                            {conv.user_profile.display_name[0]}
                          </AvatarFallback>
                        </Avatar>
                        {conv.is_online && (
                ...
                <CardHeader className="border-b">
                  <div className="flex items-center gap-3">
                    <Avatar className="w-10 h-10">
                      <AvatarImage
                        src={
                          selectedConversationData?.user_profile
                            .profile_picture_url
                        }
                      />
                      <AvatarFallback>
                        {selectedConversationData?.user_profile.display_name[0]}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      <h3 className="font-semibold">
                        {selectedConversationData?.user_profile.display_name}
                      </h3>
                  ...
                  <ScrollArea className="flex-1 p-4">
                    <div className="space-y-4">
                      {messages.map((message) => (
                        ...
                        >
                          {message.type === 'channel_approval_request' &&
                          message.approval_request ? (
                            <div className="w-full flex justify-center">
                              <ChannelApprovalMessage
                                ...
                                eventDescription={
                                  message.approval_request.event_description
                                }
                                channelName={
                                  message.approval_request.channel_name
                                }
                            ...
                            <>
                              ...
                              <div
                                className={`max-w-[70%] ${message.sender_id === userId ? 'order-first' : ''}`}
                              >
                                <div
