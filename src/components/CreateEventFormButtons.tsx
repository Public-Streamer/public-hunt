    ...
    return (
      <TooltipWrapper content={tooltipText || ''}>
        <Button
          ...
          style={{
            textShadow:
              '2px 2px 4px rgba(0,0,0,0.8), 0 0 8px rgba(255,255,0,0.5), 1px 1px 0px rgba(0,0,0,0.8)',
            fontWeight: '800',
    ...
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button
          ...
          style={
            shouldAnimate
              ? {
                  textShadow:
                    '3px 3px 6px rgba(0,0,0,0.9), 0 0 12px rgba(255,255,0,0.8), 2px 2px 0px rgba(0,0,0,1), -1px -1px 0px rgba(0,0,0,1), 1px -1px 0px rgba(0,0,0,1), -1px 1px 0px rgba(0,0,0,1)',
                  animation:
                    'shake 0.5s ease-in-out infinite, tremor 0.8s ease-in-out infinite, flicker 0.6s ease-in-out infinite, lightning 0.2s ease-in-out infinite',
                  fontWeight: '900',
                  letterSpacing: '0.5px',
                }
              : {
        ...
        >
          ...
          {shouldAnimate && (
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-yellow-400/30 to-transparent opacity-0 animate-[strobe_0.3s_ease-in-out_infinite] -skew-x-12 transform translate-x-full" />
          )}
        </Button>
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Start streaming immediately. This will create your event and go
            live. Any missing date, time, or location will be auto-populated.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            onClick={onClick}
            className="bg-green-700 hover:bg-green-800 text-yellow-50 font-black text-lg py-3"
            style={{
              textShadow:
                '3px 3px 6px rgba(0,0,0,0.9), 0 0 12px rgba(255,255,0,0.8), 1px 1px 0px rgba(0,0,0,0.8)',
              fontWeight: '800',
              letterSpacing: '0.25px',
            }}
          >
...
const CreateEventFormButtons: React.FC<CreateEventFormButtonsProps> = ({
  ...
  const getTeamTooltip = () => {
    ...
    if (
      ...
      !selectedStreamers.every((s) => s.confirmed)
    ) {
              ...
              <div className="flex justify-center w-full">
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    ...
                    />
                    <span className="text-xs sm:text-sm md:text-base lg:text-lg flex-1 text-center leading-tight break-words whitespace-normal mx-2">
