...
const LiveStreamLogo: React.FC<LiveStreamLogoProps> = ({ size = 'md', className = '' }) => {
  ...
  return (
    <div className={`relative bg-gradient-to-br from-white/30 to-white/10 backdrop-blur-sm rounded-lg border border-white/40 shadow-lg hover:shadow-xl transition-all duration-300 ${classes.container} ${className}`}>
      ...
      {/* Lightning Bolt - Middle Left */}
      <Zap className={`absolute top-1/2 -translate-y-1/2 -left-0.5 text-yellow-300 animate-pulse ${classes.accent}`} style={{ filter: 'drop-shadow(1px 1px 2px rgba(0,0,0,0.7))' }} />
      
      {/* Dollar Sign - Middle Right */}
      <DollarSign className={`absolute top-1/2 -translate-y-1/2 -right-0.5 text-green-300 animate-pulse ${classes.accent}`} style={{ animationDelay: '0.5s', filter: 'drop-shadow(1px 1px 2px rgba(0,0,0,0.7))' }} />
