...
export const CoonHuntScoreboard: React.FC<CoonHuntScoreboardProps> = ({
  ...
  const setSignedMagnitude = (
    ...
    setLocalInputValues((prev) => ({
      ...prev,
      [teamId]: { ...(prev[teamId] || {}), [valKey]: magnitude },
    }));
  };
  ...
  const setSignedSign = (
    ...
    setLocalInputValues((prev) => ({
      ...prev,
      [teamId]: { ...(prev[teamId] || {}), [signKey]: sign },
    }));
  };
  ...
  const hasAnyPending = useMemo(
    () =>
      Object.values(localInputValues).some(
  ...
  const saveAllPending = async () => {
    const entries = Object.entries(localInputValues).filter(
      ...
      setTeams((prev) =>
        prev.map((t) =>
          t.id === editingDraft.id
            ? { ...t, ...editingDraft, custom_fields: cf, score: newScore }
            : t
        )
      );
                    ...
                    <CardContent className="p-3 sm:p-3">
                      ...
                      <CollapsibleContent className="space-y-4 animate-fade-in">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:gap-4">
                          ...
                          <div className="space-y-2">
                            <Label className="text-xs sm:text-sm font-medium text-destructive block">
                              Minus Points
                            </Label>
                            {isHost ? (
                              <Input
                        ...
                        {!isHost && team.custom_fields?.warnings_notes && (
                          <div className="mt-4 space-y-2">
                            <Label className="text-xs sm:text-sm font-medium text-yellow-800">
                              Warnings/Notes:
                            </Label>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="checkbox"
                      checked={
                        editingDraft.custom_fields?.disqualified || false
                      }
                      onChange={(e) =>
                        setEditingDraft({
                          ...editingDraft,
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    ...
                    <Button
                      onClick={saveTeamChangesFromDialog}
