...
import { 
  Bell, Heart, MessageCircle, UserPlus, Calendar, Play, 
  Users, Check, Settings, Filter, MoreHorizontal, X, Star, Eye
} from 'lucide-react';
...
const Notifications: React.FC<NotificationsProps> = ({ userId }) => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'tag': return <Users className="w-4 h-4 text-purple-500" />;
      case 'follow': return <UserPlus className="w-4 h-4 text-green-500" />;
      case 'event': return <Calendar className="w-4 h-4 text-orange-500" />;
      case 'channel': return <Play className="w-4 h-4 text-pink-500" />;
  ...
  const getFilteredNotifications = () => {
    switch (filter) {
      ...
      case 'mentions':
        return notifications.filter(n => n.type === 'tag' || n.type === 'comment');
      case 'events':
        return notifications.filter(n => n.type === 'event' || n.type === 'channel');
      default:
                          ...
                          <div className="flex items-start gap-3 flex-1">
                            {notification.user_profile && (
                              <Avatar className="w-8 h-8 flex-shrink-0">
                                <AvatarImage src={notification.user_profile.profile_picture_url} />
                                <AvatarFallback>
                              ...
                              {notification.metadata && (
                                <div className="mt-2 text-xs text-muted-foreground">
                                  {notification.metadata.post_content && (
                                    <p className="italic">"{notification.metadata.post_content}"</p>
                                  )}
                                  {notification.metadata.comment_content && (
                                    <p className="italic">"{notification.metadata.comment_content}"</p>
                                  )}
                                  {notification.metadata.event_name && (
                                    <p>Event: {notification.metadata.event_name}</p>
                                  )}
                                  {notification.metadata.channel_name && (
                                    <p>Channel: {notification.metadata.channel_name}</p>
                                  )}
                                </div>
                              )}
