...
const outcomeSymbol = (o: string) =>
  o === '+' ? '+' : o === '-' ? '–' : o === 'o' ? '◯' : o === '/' ? '╱' : '…';

export const ScorecardDetails: React.FC<ScorecardDetailsProps> = ({
  dogs,
  onSave,
  canEdit = false,
  open: controlledOpen,
          ...
          <CardContent className="space-y-6">
            {dogs.length === 0 ? (
              ...
              dogs
                ...
                .map((d) => {
                  const { total, circleTotal, strikes, trees } = calcTotals(
                    d.entries
                  );
                        ...
                        {d.entries.length === 0 ? (
                          ...
                          d.entries.map((e) => {
                            ...
                            return (
                              <div
                                key={e.id}
                                className={`flex items-center justify-between text-xs border rounded p-1 ${color} ${isPending ? 'animate-pulse' : ''}`}
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  {isPending && (
                                    <Badge
                                      variant="outline"
                                      className="text-xs bg-orange-100 text-orange-800 border-orange-300 dark:bg-orange-950/50 dark:text-orange-200 dark:border-orange-800"
                                    >
                                      Pending
                                    </Badge>
                                  )}
                      ...
                      <div className="mt-3">
                        ...
                        {canEdit ? (
                          <Textarea
                            ...
                            onBlur={(e) =>
                              onSave(
                                { ...d, judgeNotes: e.currentTarget.value },
                                total
                              )
                            }
