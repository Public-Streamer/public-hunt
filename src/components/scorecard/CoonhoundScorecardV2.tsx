    ...
    <div className="space-y-4">
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60 py-2">
        <div className="flex items-center gap-2">
          <Button
            variant="secondary"
            onClick={collapseAll}
            aria-label="Collapse all boxes"
          >
            Collapse All
          </Button>
          <Button onClick={expandAll} aria-label="Expand all boxes">
            Expand All
          </Button>
        </div>
      </div>
      ...
      {!canEditScoreboard && (
        <Alert variant="info">
          ...
          <AlertDescription>
            You can view all teams and timers in real time. Only the Event
            Creator and streamers with Judge permission can add, edit, delete,
            or control timers.
          </AlertDescription>
        </Alert>
      )}
        ...
        {dogs.map((d) => (
          ...
          >
            <DogCard
              ...
              onTimerSnapshot={(dogId, snap) =>
                setTimerOverview((prev) => ({ ...prev, [dogId]: snap }))
              }
      ...
      {canEditScoreboard && (
        <div className="flex items-center gap-2">
          <Input
            placeholder="Add dog (team name)"
            value={newDog}
            onChange={(e) => setNewDog(e.target.value)}
            className="max-w-xs"
          />
          <Button onClick={addDog} disabled={loading}>
            Add
          </Button>
        </div>
      )}
