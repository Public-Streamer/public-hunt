...
export const CoonhoundScoreboardViewer: React.FC<Props> = ({
  eventId,
  isViewer = false,
}) => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setGlow((prev) => {
        ...
        let changed = false;
        for (const k in prev) {
          if (prev[k].until > now) next[k] = prev[k];
          else changed = true;
        }
  ...
  const runningDogTimers = (t: TeamRow) => {
    ...
    const serverUpdatedAt =
      (cf.timers_server_updated_at as string) || undefined;
    ...
    return Object.entries(timers)
      .filter(
        ([k, v]) => typeof v === 'object' && (v as any).status === 'running'
      )
      .map(([k, v]) => {
        ...
        const label = minutes[k as string]
          ? `${base} ${minutes[k as string]} minutes`
          : base;
        return {
          key: k,
          label,
    ...
    <div className="space-y-4">
      ...
      {!isViewer && (
        <Collapsible open={openHunt} onOpenChange={setOpenHunt}>
          <Card
            className={`glow-surface ${glow.hunt ? 'glow-active glow-warning' : ''}`}
          >
            <CardHeader className="py-3">
              <CollapsibleTrigger asChild>
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 transition-transform ${openHunt ? 'rotate-180' : ''}`}
                  />
                </Button>
              </CollapsibleTrigger>
            </CardHeader>
                  ...
                  {visibleCastBlocks.length > 0 ? (
                    visibleCastBlocks.map((b) => {
                      ...
                      return (
                        <div
                          key={b.key}
                          className={`rounded-md p-2 border ${statusCls(snap?.status || 'idle')}`}
                        >
                          <div className="flex items-center justify-between text-xs">
                            <span>
                              {b.key === 'mainHunt' &&
                              (castTimers as any).mainHuntMinutes
                                ? `Main Hunt ${(castTimers as any).mainHuntMinutes} minutes`
                                : b.label}
                            </span>
      ...
      <Collapsible open={openDetails} onOpenChange={setOpenDetails}>
        <Card
          className={`glow-surface ${glow.details ? 'glow-active glow-info' : ''}`}
        >
          <CardHeader className="py-3">
            <CollapsibleTrigger asChild>
              <Button
              ...
              >
                ...
                <ChevronDown
                  className={`h-4 w-4 transition-transform ${openDetails ? 'rotate-180' : ''}`}
                />
              </Button>
            </CollapsibleTrigger>
          </CardHeader>
