...
import type { ProcessedTimer } from "./timer";
...
interface ScorecardSummaryProps {
  dogs: DogData[];
  timerOverview: Record<
    string,
    Record<string, ProcessedTimer | DogTimerSnapshotUI>
  >;
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {activeCast.length > 0 ? (
                  activeCast.map((b) => (
                    <div
                      key={b.key}
                      className={`rounded-md p-3 border-2 ${statusCls(
                        b.status
                      )} shadow-md`}
                    >
                          ...
                          {running.map(({ key, t }) => (
                            <div
                              ...
                              className={`rounded-md p-2 border ${statusCls(
                                ...
                                t.status === "finished" ? "animate-pulse" : ""
                              }`}
                            >
                              <div className="flex items-center justify-between text-xs">
                                <span className="capitalize">{`${
                                  labelMap[key] ?? key
                                } ${durationsMin[key]} minutes`}</span>
                                <span
