...
const ProfileTimeline: React.FC<ProfileTimelineProps> = ({
  ...
  const searchUsers = async (query: string) => {
    ...
    try {
      ...
      const results = (data || [])
        .filter(
          (user) =>
            !taggedUsers.find((taggedUser) => taggedUser.id === user.user_id)
        )
        .map((user) => ({
          id: user.user_id,
    ...
    try {
        ...
        .insert({
          ...
          channel_id:
            selectedChannels && selectedChannels[0]
              ...
              : null,
          event_id:
            selectedEvents && selectedEvents[0] ? selectedEvents[0].id : null,
          metadata: {
            ...(selectedChannels &&
              selectedChannels[0] && { channel_id: selectedChannels[0].id }),
            ...(selectedEvents &&
              selectedEvents[0] && { event_id: selectedEvents[0].id }),
          },
        })
        .select()
        .single();
      ...
      const selectedEventData =
        selectedEvents && selectedEvents[0]
          ? userEvents.find((e) => e.id === selectedEvents[0].id) ||
            allEvents.find((e) => e.id === selectedEvents[0].id)
          : undefined;
  ...
  const handleGoLiveSolo = async () => {
    try {
      ...
      setTimeout(() => {
        window.location.href = `/stage/${eventResult.id}`;
      }, 2000);
    } catch (error) {
  ...
  const handleLike = async (postId: string) => {
    const updatePost = (prev: TimelinePost[]) =>
      prev.map((post) =>
        post.id === postId
          ? {
              ...post,
        ...
        prev.map((post) =>
          post.id === postId
            ? {
                ...
                content: newContent,
                media_url:
                  mediaFile === null ? undefined : mediaUrl || post.media_url, // Remove media if null, keep existing or use new
                media_type:
                  mediaFile === null
                    ? undefined
                    : mediaFile
                ...
                {expandedPost && (
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedChannels &&
                        selectedChannels.map((channel) => (
                          <div
                            key={channel.id}
                            className="flex items-center text-sm text-muted-foreground bg-muted p-2 rounded"
                          >
                            <Play className="w-4 h-4 mr-2" />
                            <span>
                              {userChannels.find((c) => c.id === channel.id)
                                ?.name ||
                    ...
                    <div className="flex flex-wrap gap-2 p-4 bg-muted/50 rounded-lg">
                                ...
                                <CommandGroup heading="Your Events">
                                  {userEvents
                                    .filter((event) =>
                                      event.name
                                        .toLowerCase()
                                        .includes(eventInput.toLowerCase())
                                    )
                                    .map((event) => (
                                      <CommandItem
                                        key={event.id}
                      ...
                      <Popover
                        open={showGoLivePopover}
                        onOpenChange={setShowGoLivePopover}
                      >
                        <PopoverTrigger asChild>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base max-w-full w-full"
                          >
                            <LiveStreamLogo
                              size="md"
                          ...
                          <div className="space-y-4">
                            <div className="text-center">
                              ...
                              <p className="text-sm text-muted-foreground mb-4">
                                Choose the admission cost for your solo
                                livestream
                              </p>
                            </div>
