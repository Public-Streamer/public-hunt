...
const ChannelRoleManager: React.FC<ChannelRoleManagerProps> = ({
  channelId,
  currentUserRole,
}) => {
  ...
  const availableRoles = [
    {
      ...
      description:
        'Full control over channel - can transfer ownership, manage all settings, and assign/remove all roles',
    },
    {
      value: 'channel_administrator',
      label: 'Channel Administrator',
      description:
        'Can manage channel settings, assign roles (except Channel Master), and moderate content',
    },
    {
      value: 'channel_manager',
      label: 'Channel Manager',
      description:
        'Can create and manage events, moderate comments, and manage basic channel operations',
    },
    {
      value: 'channel_moderator',
      label: 'Channel Moderator',
      description:
        'Can moderate comments, manage live chat, and assist with basic channel maintenance',
    },
  ];
    ...
    try {
      ...
      const subscribers =
        data?.map((sub) => {
          ...
          const profile = Array.isArray(sub.user_profiles)
            ? sub.user_profiles[0]
            : sub.user_profiles;
          return {
  ...
  const canManageRoles =
    ...
    currentUserRole === 'channel_administrator';
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {channelTeam.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant="secondary">
                        {
                          availableRoles.find((r) => r.value === user.role)
                            ?.label
                        }
                      </Badge>
                      {canManageRoles && (
              ...
              <p className="text-gray-500 text-center py-4">
                No team members assigned yet. Use the search box above to add
                channel subscribers to your production team.
              </p>
            )}
          </div>
        </CardContent>
