...
const Create: React.FC = () => {
  ...
  const isChannelFormValid = () => {
    const requiredFields = ['firstName', 'lastName', 'email', 'channelName', 'channelDescription', 'category'];
    return requiredFields.every(field => 
      channelFormData[field as keyof typeof channelFormData] !== ''
    );
  };
  
  const isEventFormValid = () => {
    const requiredFields = ['name', 'description', 'date', 'time', 'location', 'category'];
    return requiredFields.every(field => 
      eventFormData[field as keyof typeof eventFormData] !== ''
    );
  };
  ...
  const handleChannelSubmit = (e: React.FormEvent) => {
    ...
    if (isChannelFormValid()) {
      console.log('Channel creation data:', channelFormData, 'Media:', channelMedia);
      setHasChannel(true);
    }
  };
